...
static OTF2_FlushCallbacks flush_callbacks = {.otf2_pre_flush = pre_flush,
...
int open_otf2_writer()
{
    ...
    OTF2_MPI_Archive_SetCollectiveCallbacks(archive, MPI_COMM_WORLD,
                                            MPI_COMM_NULL);
...
int close_otf2_writer()
{
    ...
    uint64_t global_epoch_start;
    MPI_Reduce(&epoch_start, &global_epoch_start, 1, OTF2_MPI_UINT64_T, MPI_MIN,
               0, MPI_COMM_WORLD);

    uint64_t global_epoch_end;
    MPI_Reduce(&epoch_end, &global_epoch_end, 1, OTF2_MPI_UINT64_T, MPI_MAX, 0,
               MPI_COMM_WORLD);
